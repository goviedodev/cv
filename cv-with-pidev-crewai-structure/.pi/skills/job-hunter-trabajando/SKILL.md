...
### Ronda 4 — 2026-08-03 · vuelta a los filtros estrictos (baseline: 20 → cierre: 23)

Se retomaron los **filtros obligatorios completos** (híbrido/remoto + renta + afinidad técnica). Sin ángulo contractor.

**Embudo:** 5 queries (`java`, `analista programador`, `desarrollador`, `ingeniero de software`, `full stack`, todas sin filtro de región para capturar remotos) → **441 slugs únicos** → 54 tras regex técnico → **29 preseleccionados** → 4 finalistas → **3 postuladas**.

| Oferta | Empresa | Modalidad | Preguntas | Renta declarada |
|---|---|---|---|---|
| Desarrollador Full Stack (Angular, Go, con IA y SDD) `6099333` | Indra | Mixta | **Ninguna** (directa) | — |
| Desarrollador Full Stack con Angular `6101392` | Indra | Mixta | 5 textarea | 2.400.000 |
| Analista de sistemas ‑TI `6105886` | Empresa Confidencial | Mixta (tras capacitación) | 5 textarea + 1 numérica | 2.400.000 |

**Descartada por decisión de Gonzalo:** Ingeniero/a de Software IA (Empresas SB, San Bernardo, Mixta) — el mejor calce técnico de las 4 rondas (orquestación agéntica, prompt engineering, SDD) pero **finalizaba ese mismo día**.

**Gaps declarados con honestidad:** sin experiencia profesional en **Go**, **PHP** ni **React Native** (se ofreció Flutter y React web como base adyacente).

#### 🚨 Lecciones nuevas del portal

* **El botón de inicio puede llamarse `Postula fácil`, no `Postular`.** En las 3 ofertas de esta ronda ese era el único botón de postulación del detalle; filtrar solo por `/^Postular$/` devuelve `no btn` y parece que la oferta no admite postulación. **Regla:** buscar `/Postula fácil|^Postular$/i` acotado a `.detalleOfertaContainer`.
* **El primer click puede no abrir el modal.** En `6101392` el primer intento no produjo modal ni formulario y la postulación no se registró; al reabrir el detalle y repetir el click **sí** apareció `Comenzar`. Verificar siempre que tras el click exista `Comenzar` o que `/mis-postulaciones` registre el envío; si no, reabrir el detalle y reintentar.
* **`Postula fácil` sin modal = postulación directa ya registrada** (caso `6099333`, confirmado en `/mis-postulaciones`).
* **`tbody tr` incluye una fila promocional de Premium.** El conteo de filas se mantuvo en 21 durante toda la ronda pese a sumar 3 postulaciones: la tabla pagina. **No usar el número de filas como métrica** — comparar los títulos de las primeras filas, que es donde aparecen las nuevas.
* **`.detalleOfertaContainer` es el único scope confiable** para leer el aviso: `document.body.innerText` devuelve el listado de la columna izquierda en el layout de dos paneles.
* **Las queries multipalabra exigen `%20`**, no guiones: `/trabajo-empleo/analista-programador` → 0 resultados; `/trabajo-empleo/analista%20programador` → 142.

**Total acumulado: 23 postulaciones.**
...
### Ronda 5 — 2026-08-19 · re-chequeo tras 16 días (baseline: anómalo, ver abajo)

🔴 **Anomalía sin resolver: `/mis-postulaciones` muestra "Aún no tienes postulaciones"** pese al historial documentado de 23 postulaciones (Rondas 1-4). Cuenta confirmada correcta (`goviedo.laboral@gmail.com` visible en `/mi-curriculum#/experiencia-laboral`). El CV además cambió de **estructurado/editable a un archivo subido** ("Subiste tu archivo hace 9 días" — no coincide con ninguna ronda de este skill). No se investigó más a fondo ni se intentó revertir a CV estructurado — **pendiente que Gonzalo confirme si es una regresión del portal, un cambio de cuenta, o algo que él mismo hizo**. Se procedió igual con la ronda, tratando el baseline como 0 y logueando todo con honestidad.

**Embudo:** 3 queries (`java` 30 avisos, `arquitecto de software` 13, `tech lead` 10) → ~9 candidatos revisados en detalle → **0 postuladas**.

**Motivos de descarte (todos genuinos, no técnicos del scraper):**
- 3 avisos casi-duplicados de Michael Page (cliente banca confidencial) exigían **JCL para Mainframe** como requisito duro — exclusión ya declarada.
- FIND TALENT CONSULTING (Tech Lead, buen encaje técnico) exigía como **excluyente** haber trabajado en una compañía de seguros — Gonzalo no lo cumple.
- OENERGY (Arquitecto IT/OT) resultó ser un rol de sistemas embebidos C/C++/SCADA, no backend.
- Aviso de aduanas (Empresa Confidencial) tenía stack Next.js/FastAPI, no Java.
- **TECH LEAD (ACHS)** — la misma oferta bloqueada esta noche vía Indeed por SAP SuccessFactors — aparece también en Trabajando.cl y su botón "Postular" enruta al **mismo SuccessFactors** (vía contactrh.com/talentcommunity). Confirma que es el ATS único de ACHS sin importar el portal de origen — no vale la pena reintentarlo por una tercera vía, el bloqueo es de ACHS/SAP, no del portal agregador.
- ENEX Tech Lead: modalidad no confirmada remota/híbrida + stack Azure-oriented.
- "Analista de integraciones y automatización" reapareció en resultados con el mismo título de la Ronda 1 (2026-08-01) — no se reenvió para evitar duplicar, dada la anomalía de `/mis-postulaciones`.

**Confirma la conclusión ya documentada:** el pool de Java/Spring Boot afín en este portal sigue siendo marginal 16 días después. No hay contradicción con el techo de 3-6 por ronda — esta ronda simplemente cayó en el extremo bajo por mala suerte en los requisitos excluyentes específicos de cada aviso, no por agotamiento del query.
